...
function MediaPlane({
  ...
  const localState = React.useRef({ opacity: 0, frame: 0, ready: false, hasAppeared: false });
  ...
  useFrame(() => {
    ...
    if (instantAppear && !state.hasAppeared && target > INVIS_THRESHOLD) {
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: React Three Fiber meshes are interactive scene objects.
    <mesh
      ref={meshRef}
      position={position}
      scale={displayScale}
      ...
      onPointerOut={(event) => {
        (event.nativeEvent.target as HTMLCanvasElement).style.cursor = "grab";
      }}
    >
...
function SceneController({
  ...
  React.useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (s.isDragging) {
        s.hasDragged ||= Math.hypot(e.clientX - s.pointerStart.x, e.clientY - s.pointerStart.y) > 8;
    ...
    <>
      {chunks.map((chunk) => (
        <Chunk
          ...
          cameraGridRef={cameraGridRef}
          instantAppear={instantAppear}
          onMediaClick={(item) => !state.current.hasDragged && onMediaClick?.(item)}
        />
      ))}
    </>
  );
