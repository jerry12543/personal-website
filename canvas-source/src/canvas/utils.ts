...
export const generateChunkPlanes = (cx: number, cy: number, cz: number, mediaCount: number): PlaneData[] => {
  ...
  const mediaOffset = cx * ITEMS_PER_CHUNK + cy * 7 + cz * 9;
  ...
  for (let i = 0; i < ITEMS_PER_CHUNK; i++) {
    ...
    planes.push({
      ...
      mediaIndex: positiveModulo(mediaOffset + i, mediaCount),
    });
  }
...
export const generateChunkPlanesCached = (cx: number, cy: number, cz: number, mediaCount: number): PlaneData[] => {
  const key = `${cx},${cy},${cz},${mediaCount}`;
